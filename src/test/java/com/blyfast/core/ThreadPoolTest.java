 ...
 *
 * <p>These tests verify thread pool configuration, task execution, concurrency handling, shutdown
 * behavior, and work-stealing capabilities.
 */
...
public class ThreadPoolTest {
  ...
  private void awaitLatch(CountDownLatch latch, int timeoutSeconds) throws InterruptedException {
    assertTrue(
        latch.await(timeoutSeconds, TimeUnit.SECONDS), "Latch should complete within timeout");
  }
  ...
  void testShutdown() throws Exception {
    ...
    awaitLatch(latch, SHORT_TIMEOUT_SECONDS);
    assertTrue(
        pool.awaitTermination(SHORT_TIMEOUT_SECONDS, TimeUnit.SECONDS),
  ...
  void testShutdownNow() throws Exception {
    ...
    assertTrue(pool.getExecutor().isShutdown(), "Executor should be shutting down");
