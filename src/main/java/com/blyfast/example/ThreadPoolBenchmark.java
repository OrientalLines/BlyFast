...
public class ThreadPoolBenchmark {
  ...
  public static void main(String[] args) throws Exception {
    ...
    ThreadPool.ThreadPoolConfig optimizedConfig =
        new ThreadPool.ThreadPoolConfig()
    ...
    ThreadPool.ThreadPoolConfig workStealingConfig =
        new ThreadPool.ThreadPoolConfig()
            .setCorePoolSize(Runtime.getRuntime().availableProcessors() * 2)
    ...
    System.out.println(
        "Run a load test against /benchmark endpoint to see the thread pool in action");
   ...
   */
  private static void runBenchmark(String name, ThreadPool.ThreadPoolConfig config)
      throws Exception {
    ...
    System.out.println(
        "Throughput: " + (CONCURRENT_REQUESTS / elapsedSeconds) + " requests/second");
    ...
    if (config.isCollectMetrics()) {
      System.out.println(
          "Average task execution time: "
              + (threadPool.getAverageExecutionTime() / 1_000_000.0)
              + " ms");
    }
