...
/**
 * A highly optimized thread pool for handling HTTP requests in the BlyFast framework. This
 * implementation focuses on maximum throughput and minimal latency.
 */
public class ThreadPool {
  ...
  public ThreadPool(ThreadPoolConfig config) {
    ...
    if (config.isUseWorkStealing()) {
      ...
      logger.info(
          "Created work-stealing thread pool with parallelism level: {}", config.getCorePoolSize());
    ...
    logger.info(
        "Created thread pool with core size: {}, max size: {}, queue capacity: {}",
        config.getCorePoolSize(),
        config.getMaxPoolSize(),
  ...
  /**
   * Starts the dynamic scaling and adaptive queue monitors if they are enabled in the
   * configuration.
   */
  private void startMonitors() {
    if (executor instanceof ThreadPoolExecutor) {
      ...
      logger.info(
          "Started thread pool monitoring with dynamic scaling={}, adaptive queue={}",
          config.isEnableDynamicScaling(),
          config.isUseAdaptiveQueue());
    }
  }
  ...
  private void adjustThreadPoolSize(ThreadPoolExecutor threadPoolExecutor) {
    ...
    if (logger.isDebugEnabled()) {
      logger.debug(
          "Thread pool utilization: {}%, active: {}, size: {}, core: {}, max: {}",
    ...
    if (queue instanceof LinkedBlockingQueue) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            ...
            String.format("%.2f", queueUtilization * 100), queueSize, queueCapacity);
      }
      ...
      if (queueUtilization > 0.8) {
        ...
        logger.info(
            "Queue is heavily utilized ({}%). Will use increased capacity of {} for next startup.",
                ...
                while (!Thread.currentThread().isInterrupted()) {
                  try {
                    ...
                    if (submitted - completed > 1000) {
                      logger.warn(
                          "Work-stealing pool appears to be overloaded: {} pending tasks",
                          submitted - completed);
                    }
  ...
  /**
   * Submits a task to the thread pool and returns a Future representing the result.
   *
   * @param task the task to execute
  ...
  /**
   * Shuts down the thread pool immediately, attempting to stop all actively executing tasks.
   *
   ...
   *
   * @return the average execution time in nanoseconds, or 0 if no tasks have been completed
   */
  ...
  public static class ThreadPoolConfig {
    ...
    // Dynamic scaling - more responsive, but less aggressive to prevent oscillation
    private boolean enableDynamicScaling = true;
    private double targetUtilization = 0.90; // Increased target utilization
    private int scalingCheckIntervalMs = 1000; // Check more frequently

    // Adaptive queue behavior - more responsive
    private boolean useAdaptiveQueue = true;
    private int adaptiveQueueCheckIntervalMs = 250; // Check twice as often

    // Metrics - disable for production if not needed
    private boolean collectMetrics = false; // Disabled for maximum performance
